...
- **Take a Tour** — A public, no-login `/tour` page framed as a recruiter-facing portfolio case study: a hero with a sample scored-attempt card, the "Listen · Record · Score · Improve" loop, a raw-Azure-output vs. learner-coaching differentiator, "What I built" and "Engineering challenges" sections, grounded evidence (verified dataset facts and capabilities), an end-to-end architecture pipeline, and clear demo/source CTAs. Uses clearly-labelled sample data drawn from the demo set.
