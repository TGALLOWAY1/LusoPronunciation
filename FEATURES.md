...
- **Interactive Demo Mode** — A public, no-login `/demo` page where visitors pick from a set of real PT-BR sentences drawn from the app's content ("Oi, tudo bem?", "Estou muito feliz hoje.", "A conta, por favor.", "Minha mãe se chama Ana.", "Que horas são?"), play the native reference audio, trigger a simulated assessment, and explore a realistic sample result: an analytics-style score card (overall score, dated progress-over-time chart with a trend summary, and icon-coded sub-score meters), word-by-word chips, phoneme breakdown with IPA, and coaching tips.
