...
| --- | --- | --- | --- |
| Azure Speech – Pronunciation Assessment | Microsoft Azure Cognitive Services | **Runtime** | Scores user recordings word-by-word AND per-phoneme, and flags miscues during live practice. |
...
The core of the app. Implemented in `src/server/routes/pronunciationAssessment.ts` and `src/pipeline/azurePronunciationClient.ts`; called against `https://{region}.stt.speech.microsoft.com/.../v1?language=pt-BR&format=detailed` with a base64-encoded `Pronunciation-Assessment` config (`HundredMark`, `Phoneme` granularity, `Comprehensive` dimension, `EnableMiscue=true`). The client records webm/opus audio, the server converts it to 16 kHz mono WAV via `ffmpeg-static` (`src/server/lib/audioConversion.ts`), submits it to Azure, then normalizes the response (`src/lib/azurePronunciationNormalizer.ts`) into the internal `AttemptScore` shape containing the composite pronunciation score, accuracy, fluency, completeness, and both per-word AND per-phoneme accuracy scores. The normalizer parses `Words[].Phonemes[]` defensively across Azure's two observed shapes (flattened vs. nested `PronunciationAssessment`) and treats the phoneme *name* as optional — Azure does not guarantee phoneme names for pt-BR, so a per-phoneme SCORE is always preserved even when its `label` is `null` (the UI then shows a positional sound). Prosody is only returned by Azure for en-US, so it is absent for pt-BR and annotated as unavailable rather than substituted. Security middleware enforces per-user rate limits (20 req / 5 min), a daily quota (200 req/day), a 10 MB upload cap, and magic-byte signature validation before the request is ever forwarded to Azure.
...
`src/pipeline/azureTTSClient.ts` wraps `microsoft-cognitiveservices-speech-sdk` with retry + exponential backoff and idempotent file writes. `src/pipeline/runTTSJobs.ts` runs queued jobs with a concurrency of 4 to produce canonical WAVs at `public/audio/{words,sentences}/{ptbr_male,ptbr_female}/{id}.wav` using the voices `pt-BR-AntonioNeural` (male) and `pt-BR-FranciscaNeural` (female). Additional TTS entry points: `scripts/generateWordAudio.ts` and `src/pipeline/generateAssessmentFixtures.ts` (which also calls the Pronunciation Assessment API to build JSON+WAV test fixtures under `data/test_data/`). Invoked via `npm run audio:words` and `npm run generation:pipeline -- --stage=tts|fixtures`.
